...
if (isset($_GET['islem'])) {
    ...
    if ($islem == "sil") {
        if (isset($_GET["id"])) {
            try {
                $sorgu = $vt->prepare("delete from kategoriler where id=?");
                $sorgu->bindParam(1, $id);
                $id = $_GET["id"];
                $donen = $sorgu->execute();
                if ($donen != 0)
                    $_SESSION["durum"] = "Kategori Başarılı Bir şekilde Silindi";
                else $_SESSION["durum"] = "Kategori Silenemedi.";
            } catch (Exception $ex) {
                $_SESSION["durum"] = 0;
            }
            header("Location:kategoriler.php");
                ...
                <div class="row">
                    ...
                    <?php } else if ($durum == 2) { ?>
                        <div class="col">
                            <div class="card shadow mb-3">
                                <div class="card-header py-3">
                                    <h5 class="text-primary m-0 font-weight-bold">Kategori Ekle</h5>
                                </div>
                                <div class="card-body">
                                    <form method="post"
                                          action="kategori.php?islem=ekle">
                                        <div class="form-row">
                                            <div class="form-group"><label for="adek"><strong>Ad :
                                                    </strong></label><input class="form-control" type="text"
                                                                            id="adek"
                                                                            placeholder="Ad"
                                                                            name="adek">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group"><label
                                                        for="aciklamaek"><strong>Açıklama :</strong></label><input
                                                        class="form-control" type="text"
                                                        id="aciklamaek" placeholder="Açıklama"
                                                        name="aciklamaek">
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group"><label
                                                        for="aktifek"><strong>Aktif Mi :</strong></label><input
                                                        class="form-control" type="checkbox" id="aktifek"
                                                        name="aktifek"
                                                >
                                            </div>
                                        </div>
